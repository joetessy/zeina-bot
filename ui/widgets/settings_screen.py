...
class SettingsScreen(FloatLayout):
    ...
    def _update_system_prompt_live(self, reason="settings change"):
        ...
        if not assistant:
            return
        if hasattr(assistant, "refresh_system_prompt"):
            assistant.refresh_system_prompt(reason=reason)
        else:
            # Fallback for legacy assistants without refresh helper
            new_prompt = self._settings.get_system_prompt()
            if assistant.conversation_history and assistant.conversation_history[0].get("role") == "system":
                assistant.conversation_history[0]["content"] = new_prompt
                assistant._obs("lite", f"System prompt updated ({len(new_prompt)} chars)")
                assistant._obs("verbose", f"System prompt: ({new_prompt})")
    ...
    def _on_clear_history(self):
        ...
        if assistant:
            ...
            if hasattr(assistant, "refresh_system_prompt"):
                assistant.refresh_system_prompt(reason="history cleared")
            else:
                prompt = self._settings.get_system_prompt()
                assistant.conversation_history.append({"role": "system", "content": prompt})
    ...
    def _on_profile_switch(self, instance, name):
        ...
        if hasattr(self._app, '_assistant') and self._app._assistant:
            ...
            if hasattr(assistant, "refresh_system_prompt"):
                ...
                assistant.conversation_history.append({"role": "system", "content": system_prompt})
